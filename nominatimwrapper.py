...
from config import osm_host, libpostal_host, photon_host, default_transformers_sequence
...
namespace = Namespace(
    'nominatimwrapper',
    'A geocoder built upon Nominatim')
...
single_parser.add_argument('with_rejected',      type=str, choices=('yes', 'no'), help='if "yes", rejected results are returned (default: "no")')
single_parser.add_argument('check_result',       type=str, choices=('yes', 'no'), help='if "yes", will "double check" OSM results (default: "yes")')
single_parser.add_argument('struct_osm',         type=str, choices=('yes', 'no'), help='if "yes", will call the structured version of OSM (default: "no")')
single_parser.add_argument('extra_house_nbr',    type=str, choices=('yes', 'no'), help='if "yes", will call libpostal on all addresses to get the house number  (default: "yes")')
...
class search(Resource):
    ...
    def get(self):
        ...
        - match: a single result, with the following fields: 
            - Results comming straight from Nominatim: 
             ...
             - Check results indicators (if check_result='yes'):
                ...
                - SIM_house_nbr
             - Other information:
                - method: which transformation method was used before sending the address to Nominatim. If the address was found without any transformation, will be "orig" (or "fast")
                - osm_addr_in: which address (after possibly some sequence of transformation)
        ...
        check_results = get_yesno_arg("check_result", "yes")
...
batch_parser.add_argument('check_result',       type=str, choices=('yes', 'no'), help='if "yes", will "double check" OSM results (default: "yes")')
...
class batch(Resource):
    ...
    def post(self):
        ...
        -------
        str
            json containing geocoded addresses. TO BE DETAILLED
